...
export class ConversationHeader {
  ...
  constructor(container: HTMLElement) {
    ...
    this.togglePanelBtn = this.element.querySelector(".toggle-panel-btn") as HTMLButtonElement;
    this.projectSelector = this.element.querySelector("#project-selector") as HTMLSelectElement;
    this.newProjectBtn = this.element.querySelector(".new-project-btn") as HTMLButtonElement;
    this.manageProjectsBtn = this.element.querySelector(".manage-projects-btn") as HTMLButtonElement;
    ...
    this.manageProjectsHandler = () => {
      console.log("Manage projects button clicked");
      projectStore.showProjectManager();
    };
    
    this.projectSelectorHandler = () => {
      const selectedId = this.projectSelector.value;
      console.log("Project selector changed:", selectedId);
      if (selectedId) {
        projectStore.setActiveProject(selectedId);
      }
    };
    
    if (!this.togglePanelBtn || !this.projectSelector || !this.newProjectBtn || !this.manageProjectsBtn) {
  ...
  private initialize(): void {
    ...
    this.manageProjectsBtn.addEventListener("click", this.manageProjectsHandler);
  ...
  private setupProjectSelector(): void {
    ...
    this.projectSelector.addEventListener("change", this.projectSelectorHandler);
  }
  ...
  private setupEffects(): void {
    ...
    this.cleanupFns.push(
      effect(() => {
        ...
        this.togglePanelBtn.classList.toggle("active", layout === "left-expanded");
      })
    );
  }
  
  private updateProjectOptions(projects: Array<{id: string, name: string}>): void {
  ...
  public destroy(): void {
    ...
    this.manageProjectsBtn.removeEventListener("click", this.manageProjectsHandler);
    this.projectSelector.removeEventListener("change", this.projectSelectorHandler);
